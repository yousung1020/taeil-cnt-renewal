    ...
    <>
      ...
      <motion.header 
        ...
        animate={{
          ...
          borderBottom: isOpen ? '1px solid rgba(255,255,255,0)' : '1px solid rgba(255,255,255,0.1)',
        }}
      ...
      >
        <div className="flex items-center justify-between px-6 md:px-12 py-4">
          ...
          <div className="flex items-center">
            <button onClick={toggleMobileMenu} className="text-white hover:text-green-400 focus:outline-none z-[70] p-2">
              <div className="w-8 h-6 flex flex-col justify-between relative">
                <motion.span animate={isOpen ? { rotate: 45, y: 11 } : { rotate: 0, y: 0 }} className="w-full h-0.5 bg-white block rounded-full origin-center" />
                <motion.span animate={isOpen ? { opacity: 0 } : { opacity: 1 }} className="w-full h-0.5 bg-white block rounded-full" />
                <motion.span animate={isOpen ? { rotate: -45, y: -11 } : { rotate: 0, y: 0 }} className="w-full h-0.5 bg-white block rounded-full origin-center" />
              </div>
            </button>
          </div>
                ...
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-10">
                  ...
                  <div className="flex flex-col sm:flex-row lg:flex-col items-center sm:items-end gap-6 w-full sm:w-auto">
                    ...
                    <div className="relative w-full sm:w-auto">
                      ...
                      >
                        <span className="text-[14px] font-medium uppercase tracking-wider">Family Site</span>
                        <svg className={`w-4 h-4 transition-transform ${isFamilySiteOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        {isFamilySiteOpen && (
                          ...
                          >
                            {familySites.map(site => (
                              <a key={site.name} href={site.url} className="block px-4 py-3 text-[14px] text-white/70 hover:text-white hover:bg-green-900/50 transition-all border-b border-white/5 last:border-0">{site.name}</a>
                            ))}
                          </motion.div>
                        )}
                      </AnimatePresence>
